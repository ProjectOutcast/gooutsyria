/**
 * Demo seed for Go Out Syria.
 * All restaurants/reviews are fictional demo content — replace with real data
 * before public launch. Demo accounts are printed at the end.
 */
...
async function main() {
  ...
  const damascus = await db.city.create({
    data: { slug: "damascus", nameAr: "دمشق" },
  });
  ...
  const neighborhoods = await Promise.all(
    neighborhoodNames.map((nameAr) =>
      db.neighborhood.create({
        data: { nameAr, slug: slugify(nameAr), cityId: damascus.id },
      })
    )
  );
  const nb = (name: string) => neighborhoods.find((n) => n.nameAr === name)!.id;

  const cuisineDefs: [string, string, string][] = [
    ["شامي أصيل", "shami", "🥙"],
    ["مشاوي", "mashawi", "🍢"],
    ["إيطالي", "italian", "🍕"],
  ...
  const cuisines = await Promise.all(
    cuisineDefs.map(([nameAr, slug, icon]) =>
      db.cuisine.create({ data: { nameAr, slug, icon } })
    )
  );
  const cu = (slug: string) => cuisines.find((c) => c.slug === slug)!.id;

  const featureDefs: [string, string, string][] = [
    ["واي فاي", "wifi", "📶"],
    ["جلسة خارجية", "outdoor", "🌿"],
    ["صالة عائلات", "family", "👨‍👩‍👧"],
    ["أركيلة", "shisha", "💨"],
  ...
  const features = await Promise.all(
    featureDefs.map(([nameAr, slug, icon]) =>
      db.feature.create({ data: { nameAr, slug, icon } })
    )
  );
  ...
  const [, owner, ...reviewers] = await Promise.all([
    ...
    ...[
      ["لين الجابري", "user@gooutsyria.com", "User1234!"],
      ["سامر الحمصي", "samer@example.com", "Demo1234!"],
      ["رنا الشامية", "rana@example.com", "Demo1234!"],
      ["خالد العبد الله", "khaled@example.com", "Demo1234!"],
      ["نور حداد", "nour@example.com", "Demo1234!"],
      ["مجد السباعي", "majd@example.com", "Demo1234!"],
    ].map(async ([name, email, pw]) =>
      db.user.create({
        data: { name, email, passwordHash: await bcrypt.hash(pw, 10) },
      })
    ),
  ]);
  ...
  type RestDef = {
    ...
    photos: number[]; // placeholder indices
    menu: SectionDef[];
    verified?: boolean;
    pro?: boolean;
    owned?: boolean;
  ...
  const defs: RestDef[] = [
    ...
    },
    {
      ...
      desc: "فول، فتة حمص، مسبحة، وقلاية بندورة — فطور شامي كامل من السادسة صباحاً، مع خبز مشروح طازع من الفرن المجاور.",
      nbName: "القيمرية",
  ...
  for (const def of defs) {
    const r = await db.restaurant.create({
      data: {
        slug: slugify(def.nameEn ?? def.nameAr),
        nameAr: def.nameAr,
        nameEn: def.nameEn,
        ...
        photos: {
          create: def.photos.map((p, i) => ({
            url: `/placeholders/p${p}.svg`,
            alt: def.nameAr,
            kind: i === 0 ? "EXTERIOR" : i === 1 ? "INTERIOR" : "FOOD",
            sortOrder: i,
          })),
        },
        menuSections: {
  ...
  // --- Reviews (approved, deterministic spread) ---
  ...
  for (const r of restaurants) {
    const count = 3 + (ri % 5); // 3..7 reviews each
    for (let i = 0; i < count && i < reviewers.length; i++) {
      const [rating, text] = reviewTexts[(ri + i) % reviewTexts.length];
    ...
    await db.restaurant.update({
      ...
      data: {
        ...
        viewCount: 150 + ri * 87,
        callClicks: 12 + ri * 5,
        whatsappClicks: 8 + ri * 3,
        directionClicks: 5 + ri * 2,
      },
    });
    ri++;
  }

  // one pending review for the moderation demo
  await db.review.create({
    data: {
      restaurantId: restaurants[0].id,
      userId: reviewers[5].id,
      rating: 5,
      text: "زرت المطعم البارحة وكانت السهرة ولا أحلى — الفتة الشامية عندهم ما إلها مثيل!",
      status: "PENDING",
    },
  });
  ...
  // --- Featured placements (the monetization demo) ---
  await db.featuredPlacement.createMany({
    data: [
      { restaurantId: restaurants[0].id, slot: "HOME", startsAt: inDays(-5), endsAt: inDays(25), notes: "باقة شهرية — مدفوع نقداً" },
      { restaurantId: restaurants[12].id, slot: "HOME", startsAt: inDays(-5), endsAt: inDays(25), notes: "باقة شهرية" },
      { restaurantId: restaurants[8].id, slot: "SEARCH", startsAt: inDays(-5), endsAt: inDays(25), notes: "أعلى نتائج البحث — شهر" },
      { restaurantId: restaurants[3].id, slot: "CUISINE", cuisineId: cu("italian"), startsAt: inDays(-5), endsAt: inDays(25), notes: "صفحة المطبخ الإيطالي" },
    ],
  });

  // --- Sponsor (brand ads demo) ---
  ...
  // --- Daily metrics for the owner dashboard demo ---
  ...
  console.log(`
✓ seeded: ${restaurants.length} restaurants, ${neighborhoodNames.length} neighborhoods, ${cuisineDefs.length} cuisines, 3 collections, 5 offers
