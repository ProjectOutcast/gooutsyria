...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const e = await getEventBySlug(city, slug);
  if (!e) return {};
  return {
...
export default async function EventDetail({ params }: Props) {
  ...
  const now = new Date();
  const similar = await getSimilarEvents(city, e.category, e.id, now, 4);
  ...
  return (
    <div className="max-w-[1180px] mx-auto px-4 sm:px-7 pb-28 lg:pb-16">
      ...
      <nav className="hidden sm:flex items-center gap-1.5 text-[13.5px] text-[#9A887F] pt-5" aria-label="مسار التنقل">
        ...
        <Link href={`/${city}/events`} className="hover:text-primary-500">الفعاليات</Link>
        <Chevron dir="left" size={13} />
        <Link href={`/${city}/events#calendar`} className="hover:text-primary-500">{catAr}</Link>
        <Chevron dir="left" size={13} />
