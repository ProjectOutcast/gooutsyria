...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!cityRow) return {};
  return {
    title: `وين نروح؟ — فعاليات وحفلات ${cityRow.nameAr}`,
    description: `دليل فعاليات ${cityRow.nameAr}: حفلات، مسرح، مهرجانات، معارض ورياضة — تصفّح حسب اليوم واعرف أماكن شراء التذاكر. منصّة ترويجية فقط، بدون شراء أونلاين.`,
...
export default async function EventsPage({ params }: Props) {
  ...
  const [featured, windowed] = await Promise.all([
    getFeaturedEvents(city, now),
    getEventsInWindow(city, now, 8),
  ]);
  ...
  return (
    <div className="pb-16">
      <h1 className="sr-only">وين نروح؟ — فعاليات وحفلات {cityRow.nameAr}</h1>
      ...
      <section className="pt-10">
        <div className="relative overflow-hidden rounded-[22px] bg-white border border-[#F2DCB0] text-center p-8 sm:p-[46px]">
          ...
          <div className="relative">
            <h2 className="text-[24px] sm:text-[30px] font-bold text-ink">تنظّم فعالية في {cityRow.nameAr}؟</h2>
